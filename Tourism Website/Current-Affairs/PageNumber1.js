...
function handlePageNavigation(targetPage) {
    ...
    const isOnNewsRegistered = currentPath.includes('news.html');
    const isOnNewsPage2 = currentPath.includes('newspage2.html');

    if (isOnNewsRegistered && targetPage === 2) {
        window.location.href = 'newspage2.html';
        return true;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.location.pathname.includes('newspage2.html')) {
        currentPage = 2;
    }
    updatePagination();
});
